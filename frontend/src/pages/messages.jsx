...
const Messages = () => {
  ...
  // fetch lawyers + init lastSeenRef on first load
  useEffect(() => {
    fetch(`${BASE}/messages/lawyers-list`)
      ...
      .then(async (data) => {
        ...
        for (const lawyer of data) {
          ...
          try {
            const res  = await fetch(`${BASE}/messages/${myId}/${lawyer.lawyer_id}`);
  ...
  // background polling for unread + notifications
  useEffect(() => {
      ...
      try {
        ...
        for (const lawyer of lawyersList) {
          ...
          if (msgData.length > 0) {
            ...
            // unread = messages from them after i last opened this chat
            const messagesFromThem = msgData.filter(
              m => String(m.sender_id) === String(lawyer.lawyer_id)
            );
            const lastReadTime = lastReadRef.current[lawyer.lawyer_id]
              ? new Date(lastReadRef.current[lawyer.lawyer_id])
              : null;

            if (lastReadTime) {
              const unread = messagesFromThem.filter(
                m => new Date(m.created_at) > lastReadTime
              );
              if (unread.length > 0) newUnreadCounts[lawyer.lawyer_id] = unread.length;
            }

            // notification trigger
            const lastSeenTime = lastSeenRef.current[lawyer.lawyer_id];
            const isNew =
              lastSeenTime &&
              String(lastMsg.sender_id) === String(lawyer.lawyer_id) &&
              new Date(lastMsg.created_at) > new Date(lastSeenTime);

            if (isNew) showNotification(lawyer, lastMsg);
          ...
          {sortedLawyers.map((lawyer) => {
            ...
            return (
              <div key={lawyer.lawyer_id}
                ...
                {isActive && (
                  <div className="flex-shrink-0 rounded-circle" style={{ width: '8px', height: '8px', background: '#22c55e' }} />
                )}
              </div>
            );
          })}
        ...
        {selectedUser ? (
          <>
            ...
            <div className="flex-grow-1 px-4 py-3"
              style={{ overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {Object.entries(groupedMessages).map(([date, msgs]) => (
                <div key={date}>
                  ...
                  {msgs.map((msg) => {
                    ...
                    return (
                      <div key={msg.message_id} className="d-flex mb-2"
                        ...
                        <div style={{ maxWidth: '65%' }}>
                          ...
                          <div style={{
                            fontSize: '10px', color: timeColor, marginTop: '4px',
              ...
              {messages.length === 0 && !loading && (
                <div className="text-center my-auto" style={{ fontSize: '14px', color: textSub }}>
            ...
            <div className="px-4 py-3 flex-shrink-0"
              ...
              <div className="d-flex align-items-end gap-3 rounded-4 px-4 py-3"
                style={{ background: inputBarBg, border: `1px solid ${inputBarBdr}` }}>
                <textarea
                  rows={1}
                  value={newMsg}
